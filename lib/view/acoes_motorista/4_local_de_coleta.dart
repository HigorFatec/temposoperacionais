...
class LocalDeColetaScreen extends StatefulWidget {
  @override
  State<LocalDeColetaScreen> createState() => LocalDeColetaScreenState();
}

class LocalDeColetaScreenState extends State<LocalDeColetaScreen> {
  ...
  String placaCarreta = '';
  String transportadora = '4.00 Chegada no Cliente';
  String tipo = 'inicio';
  // Status atual
  String? statusAtual;
  String? ordemAtual;
  ...
  Future<void> inicio_da_viagem() async {
    try {
      final snapshot = await FirebaseFirestore.instance
          .collection(isParadaExtra ? 'parada_extra_4' : 'local_de_coleta')
          .orderBy('codigo')
          .get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Local de Coleta'),
        actions: [
          IconButton(
            onPressed: () {
              Navigator.pushReplacementNamed(context, '/local_de_coleta');
            },
